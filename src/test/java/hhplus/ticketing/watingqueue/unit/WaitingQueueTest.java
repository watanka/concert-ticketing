...
import hhplus.ticketing.domain.watingqueue.infra.MemoryTokenRepository;
...
public class WaitingQueueTest {
    ...
    WaitingQueueService waitingQueueService = new WaitingQueueService(tokenGenerator, tokenRepository);

    @Test
    @DisplayName("없는 토큰을 조회할 경우 예외 처리")
    void query_invalid_token(){
        Token invalidToken = new Token("INVALID-TOKEN", 1, LocalDateTime.now());
    ...
    void receive_token_when_register_in_waiting_queue_with_userID(){
        ...
        Token token = waitingQueueService.register(userId);
    ...
    void query_waiting_number_by_token(){
        ...
        Token token1 = waitingQueueService.register(userId1);
        Token trgToken = waitingQueueService.register(userId2);
    ...
    void deactivate_token_when_time_up(){
        ...
        Token token = new Token("", 1, timeAlreadyExpired);
        token.updateStatus(TokenStatus.WAITING);


        assertThat(tokenManager.checkExpired(token)).isTrue();

        tokenManager.deactivate(token);
