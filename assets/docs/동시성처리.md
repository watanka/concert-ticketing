- [ ]  낙관적락은 충돌이 적을 것을 예상하고, 버젼정보 불일치시 예외를 발생시킨다. 예외 발생시, 보통 재시도 처리를 하기도 하는데, 낙관적락에서 그냥 재시도없이 fail 시키면 끝 아닌가?
    - [ ]  예외처리에서 그냥 실패했습니다 뜨면 안되나?
- [ ]  Pessimistic Lock의 READ, WRITE 사용 차이
    - [ ]  Lock을 JPA 함수 하나에다가 붙이는데, 그럼 다른 함수들은 Lock이 없어도 트랜잭션 수행이 가능한건가? 아니면 하나의 함수에 Lock을 붙이면 모든 함수들이 락을 대기해야하는가?

      예를 들어,
      @Lock Seat findById 설정

      그럼 findById 사용마다 Seat 로우에 락이 걸릴텐데,

      그럼 다른 jpa함수 findBySomethingElse들은 그 락을 대기했다가 획득해야한다는 건가?

      그렇게 되면 Seat Repository에 있는 어떤 함수던 Lock 대기를 해야한다는 뜻 아닌가?
...
먼저 동시성 이슈가 발생할만한 API는 좌석 예약과 포인트 충전이다.
...
class ReservationService(){
	public ReservationResponse reserve(ReservationRequest request){
		request에서 유저아이디, 좌석아이디, 콘서트아이디를 읽어온다
		
		좌석 검증.orElseThrow() // 좌석 예약 상태를 확인한다.
		좌석 조회(좌석아이디)
		좌석 상태 업데이트
		예약 생성(유저아이디, 좌석아이디, 콘서트아이디, ...)
...
- 낙관적 락
    ...
    - 단점은 낙관적 락은 기본적으로 트랜잭션 충돌이 적을 것을 가정한다. 버젼을 수정한 최초 하나의 트랜잭션 빼고는, 버젼 불일치로 나머지 트랜잭션은 전부 다 실패하게 되기 때문에, 예외처리가 필수적이다. 만약 예외처리가 났을 때, 값을 다시 수정하는 시도를 한다고 하면, 몇 번 이상 실패할지 보장할 수 없다.
      ...
      ⇒ retry가 많아지면 자연스럽게 DB connection 시도가 많아지게 되고, 성능 저하로 이어진다.
